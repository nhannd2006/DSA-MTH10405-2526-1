...
void insertArray(int arr[], int *n, int value, int index, int *assigns);
...
int main() {
    ...
    for (int k = 0; k < K; k++) {
        int freq[N + 2] = {0}; //because the worst case of insertion is N+1 steps
        for (int i = 0; i < TIMES[k]; i++) {
            int nTemp = n;
            initializeRandomArray(arr, nTemp, min, max);
            int index = rand() % (nTemp + 1); //index = nTemp when inserting at the array's end
            insertArray(arr, &nTemp, TIMES[k], index, &assigns);
            freq[assigns]++;
        }
